...
cd('C:\Users\mattLab\Dropbox (Brown)\sharedMatlabUtilities\colorTools')
...
% Get these from CALIBRATION_426C2022.xlsx
xR=.624      %monitorData.xR; % x chromaticity value for red channel
yR=.328    %monitorData.yR; % y chormaticity value for red channel
xG=.3    %monitorData.xG;
yG=.598       %monitorData.yG;
xB=.153     %monitorData.xB;
yB=.064       %monitorData.yB;

% slope and intercept from excel spreadsheet (get from graphs!)
constantR= 1.6351                 %monitorData.constantR
slopeR=    .528                  %monitorData.slopeR
constantG= 1.4152                   %monitorData.constantG
slopeG=    .4947          %monitorData.slopeG
constantB=  1.7809  %       monitorData.constantB
slopeB=     .6101    %monitorData.slopeB

% xR=.656      %monitorData.xR; % x chromaticity value for red channel
% yR=.334     %monitorData.yR; % y chormaticity value for red channel
% xG=.320      %monitorData.xG;
% yG=.610       %monitorData.yG;
% xB=.151     %monitorData.xB;
% yB=.063       %monitorData.yB;
% 
% % slope and intercept from excel spreadsheet (get from graphs!)
% constantR= 1.5967                 %monitorData.constantR
% slopeR=    .4721                  %monitorData.slopeR
% constantG= 1.4512                   %monitorData.constantG
% slopeG=    .4191          %monitorData.slopeG
% constantB=  1.7824  %       monitorData.constantB
% slopeB=     .4605    %monitorData.slopeB
% 
% 
...
A_offset=0;
B_offset=0;
gain=50;    % If you get an error that says the circle is out of range, 
...
whitePoint=[110, 100, 90]
[XYZ] = lab2xyz([L' A' B' ], 'user', whitePoint) % using one of "default" white points... check to make sure it looks neutral gray
...
allStim=[[rVals, gVals, bVals]; [Rgray, Ggray, Bgray]; [Rw, Gw, Bw]];
...
% 
% Y_reading_=Y_reading_raw(1:end);
% x_reading_=x_reading_raw(1:end);
% y_reading_=y_reading_raw(1:end);



Y_reading=Y_reading_raw(1:end);
x_reading=x_reading_raw(1:end);
y_reading=y_reading_raw(1:end);
...
hold on
plot(A, B, '.')
plot(A(sel), B(sel), 'oc', 'markerSize', 14)

%plot(lab(:,2), lab(:,3), '.r')

plot(lab(1:end-2,2), lab(1:end-2,3), 'or', 'markerFaceColor', 'r', 'markerEdgeColor','k', 'markerSize', 16, 'lineWidth', 1)
...
ylabel('A*')
xlabel('B*')
saveas(gcf, sprintf('calibrationFigure_complete%s', dateStr));
...
save(sprintf('labNewMonitorData2_%s_426C.mat', Date), 'monitorData');

fn=sprintf('colorCalibrationData2_%s_426C.mat', Date);
save(fn)
